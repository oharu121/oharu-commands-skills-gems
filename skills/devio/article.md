...
2. **AskUserQuestion ツール**を使い、以下の2つの質問を**1回のツール呼び出し**でまとめて表示する:
   ...
   - **質問2: タグ選択**（複数選択、multiSelect: true）
     - タグは Contentful のマスターリストから選ぶ必要がある（自由入力のタグは公開時にエラーになる）
     - まず `python3 ${CLAUDE_SKILL_DIR}/scripts/contentful.py tags --search <keyword>` をトピックに関連するキーワード2-3個で実行し、実在するタグを検索する
     - タグキャッシュが存在しない場合は、先に `python3 ${CLAUDE_SKILL_DIR}/scripts/contentful.py tags --refresh` を実行する
     - 検索結果から最も関連性の高い3-4個のタグを選択肢として提示する
     - ユーザーは「その他」で自由入力もできるが、Contentfulに存在するタグ名と完全一致する必要がある旨を注記する
...
- **articleId**: (optional, auto-populated) Contentfulのエントリ ID。`/devio publish` で初回公開時に自動で追記される。手動で設定しないこと
